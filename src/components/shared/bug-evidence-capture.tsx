...
export function BugEvidenceCapture({ ticketId, bugId, value, onChange }: BugEvidenceCaptureProps) {
  ...
  function renderAnnotation(annotation: RetestFrame['annotations'][number]) {
    ...
    if (annotation.type === 'arrow') {
      return <ArrowDown className="absolute h-7 w-7 -translate-x-1/2 -translate-y-1/2 text-accent drop-shadow-[0_0_10px_rgba(56,189,248,0.35)]" style={style} />
    }
  ...
  function renderPlayerViewer(mode: 'default' | 'expanded') {
    ...
    return (
      <div className={cn('space-y-4 rounded-[28px] border border-border bg-black/20', isModal ? 'w-full max-w-6xl p-6 shadow-[0_0_60px_rgba(0,0,0,0.45)]' : 'p-5')}>
        ...
        <div className={cn('overflow-hidden rounded-[28px] border border-border bg-[radial-gradient(circle_at_top,rgba(56,189,248,0.08),transparent_55%),rgba(0,0,0,0.45)]', isModal ? 'min-h-[70vh]' : 'min-h-[24rem]')}>
          <div key={mediaKey}>
            {currentPlayerFrame ? (
