...
import {
  deleteDemandaScenarioFrame,
  saveDemandaScenarioFrame,
  updateDemandaScenarioFrameMetadata,
} from '@/services/demandas-api'
import type { DemandaCenarioRecord, RetestFrame } from '@/types/domain'

interface DemandaScenarioEvidenceCaptureProps {
  demandaId: string
  tarefaId: string
  scenario: DemandaCenarioRecord
  onChange: (nextScenario: DemandaCenarioRecord) => void
}
...
function FrameImage({
  ...
  const fallbackSource =
    frame.imageUrl && frame.imageUrl !== primarySource
      ? frame.imageUrl
      : frame.downloadUrl && frame.downloadUrl !== primarySource
        ? frame.downloadUrl
        : ''
...
export function DemandaScenarioEvidenceCapture({
  demandaId,
  tarefaId,
  scenario,
  onChange,
}: DemandaScenarioEvidenceCaptureProps) {
  ...
  const [message, setMessage] = useState(
    scenario.frames?.length
      ? 'Quadros do cenario ja foram persistidos. Se quiser, carregue um GIF para extrair novos momentos.'
      : 'Suba um GIF do cenario para pausar, navegar e extrair os quadros relevantes do fluxo.',
  )
  ...
  useEffect(() => {
    if (!selectedFrameId && frames[0]?.id) {
  ...
  async function buildPlayerFrames(file: File) {
    ...
    if (!ctx || !patchCtx) {
      throw new Error('Nao foi possivel iniciar o player de GIF deste cenario.')
    }
    ...
    return patchFrames.map((frame, index) => {
      ...
      const renderedFrame = {
        id: `demanda-scenario-player-frame-${Date.now()}-${index + 1}`,
  ...
  async function handleGifUpload(file?: File) {
    ...
    setIsPlaying(false)
    setMessage(`${nextPlayerFrames.length} quadros carregados no player. Pause no ponto certo e extraia os momentos relevantes.`)
  ...
  async function extractCurrentFrame() {
    ...
    try {
      ...
      const storedFrame = await saveDemandaScenarioFrame({
        demandaId,
        tarefaId,
      ...
      setMessage(`Quadro ${nextFrames.length} adicionado ao cenario.`)
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Nao foi possivel salvar o quadro do cenario.')
    } finally {
  ...
  async function deleteCapturedFrame(frameId: string) {
    ...
    try {
      await deleteDemandaScenarioFrame(demandaId, tarefaId, scenario.id, frameToDelete.fileName)
      ...
      setMessage('Quadro removido do cenario.')
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Nao foi possivel remover o quadro do cenario.')
    }
  }
  ...
  async function persistFrameMetadata(frame: RetestFrame, description: string) {
    ...
    try {
      await updateDemandaScenarioFrameMetadata(demandaId, tarefaId, scenario.id, frame.fileName, {
        description,
        timestampLabel: frame.timestampLabel,
      })
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Nao foi possivel atualizar a descricao do quadro.')
    }
  }
  ...
  return (
    <Card className="space-y-4 border border-accent/15 bg-accent/5">
      <div>
        <p className="text-sm text-muted">Narrativa visual do cenario</p>
        <h5 className="font-display text-lg font-bold text-foreground">GIF e quadros do cenario</h5>
      ...
      <label className="rounded-3xl border border-dashed border-accent/25 bg-black/10 p-4">
        ...
        <div className="flex cursor-pointer items-center gap-3 text-foreground">
          ...
          <div>
            <p className="font-semibold">{scenario.gifName || 'Selecionar GIF do cenario'}</p>
            <p className="text-sm text-muted">O GIF fica local nesta etapa; os quadros extraidos sao persistidos na demanda.</p>
          </div>
        </div>
      </label>
      ...
      <div className="overflow-hidden rounded-[28px] border border-border bg-[radial-gradient(circle_at_top,rgba(56,189,248,0.08),transparent_55%),rgba(0,0,0,0.45)]">
        {currentPlayerFrame ? (
          <img src={currentPlayerFrame.dataUrl} alt="Quadro atual do cenario" className="h-[20rem] w-full object-contain" />
        ) : scenario.gifPreviewUrl ? (
          <img src={scenario.gifPreviewUrl} alt="Preview do GIF do cenario" className="h-[20rem] w-full object-contain" />
        ) : (
          <div className="flex h-[20rem] items-center justify-center px-6 text-center text-sm text-muted">
            O player do cenario aparecera aqui apos o upload do GIF.
          </div>
        )}
      </div>
            ...
            <div className="relative min-h-[320px] overflow-hidden rounded-3xl border border-border bg-black/30">
              {selectedFrame ? (
                ...
                <div className="flex h-[320px] items-center justify-center text-sm text-muted">
                  Selecione um quadro do cenario para revisar.
                </div>
              )}
            </div>
            <label className="space-y-2">
              <span className="text-sm font-semibold text-foreground">Descricao do quadro</span>
              <textarea
                ...
                placeholder="Explique rapidamente o que este quadro comprova no cenario."
              />
            </label>
          </div>
        </div>
